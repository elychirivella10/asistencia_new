        ...
        try:
            with psycopg2.connect(self.db_url) as conn:
                with conn.cursor() as cur:
                    cur.execute("""
                        SELECT
                            u.id, u.email, u.nombre, u.excluir_tardanza,
                            t.hora_entrada, t.margen_tolerancia_min,
                            a.id as area_id, a.jefe_id, a.parent_id, a.excluir_tardanza as area_excluir_tardanza
                        FROM usuarios u
                    ...
                    u_id, email, nombre, u_excluir, hora_entrada, tolerancia, area_id, jefe_id, parent_id, a_excluir = user_data

                    # Comprobar excepciones de llegada tardía 
                    if u_excluir or a_excluir:
                        print(f"🛡️ Excepción activa: Omitiendo notificación para {nombre} ({cedula})")
                        return None
