...
def procesar_notificacion_tardanza(db_repo, cedula, timestamp):
    """Verifica si es tarde y envía el correo si aplica (exclusivamente al supervisor)."""
    try:
        late_info = db_repo.get_late_arrival_info(cedula, timestamp)
        
        # Validar si es tarde y si existe un supervisor al que notificar
        if late_info and late_info.get("es_tarde") and late_info.get("supervisor_email"):
            exito = send_late_arrival_email(
                late_info["supervisor_email"],
                late_info["nombre"],
                timestamp,
                late_info["hora_entrada"]
            )
            # Loguear el resultado del envío (destinatario es el supervisor)
